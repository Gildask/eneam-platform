...
export default function AddSemestreForm({ niveaux }: { niveaux: Niveau[] }) {
  const router = useRouter()
  const [open, setOpen] = useState(false)
  const [loading, setLoading] = useState(false)
  const [error, setError] = useState('')
  const [form, setForm] = useState({ code: '', nom: '', niveau_id: '', ordre: '1' })
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const res = await fetch('/api/admin/semestres', {
      method: 'POST',
    ...
    setForm({ code: '', nom: '', niveau_id: '', ordre: '1' })
    setOpen(false)
    setLoading(false)
    router.refresh()
  }
        ...
        <form onSubmit={handleSubmit} className="border-t border-gray-100 px-5 py-4 space-y-3">
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="col-span-2">
              <label className="text-xs font-medium text-gray-600 mb-1 block">Nom du semestre *</label>
              <input name="nom" value={form.nom} onChange={handleChange} required
                className="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                placeholder="Semestre 1" />
            </div>
            <div>
              <label className="text-xs font-medium text-gray-600 mb-1 block">Ordre (1, 2...)</label>
