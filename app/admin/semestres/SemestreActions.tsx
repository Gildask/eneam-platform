...
type Semestre = { id: string; code: string; nom: string; ordre: number }

export default function SemestreActions({ semestre }: { semestre: Semestre }) {
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    setError('')
    setLoading(true)
    const res = await fetch(`/api/admin/semestres/${semestre.id}`, {
      method: 'PATCH',
  ...
  async function handleDelete() {
    setLoading(true)
    const res = await fetch(`/api/admin/semestres/${semestre.id}`, { method: 'DELETE' })
      ...
      <td colSpan={4} className="px-4 py-3 bg-blue-50">
        <form onSubmit={handleSave} className="flex flex-wrap gap-2 items-end">
          ...
          <div>
            ...
            <input name="ordre" type="number" min="1" value={form.ordre} onChange={handleChange}
      ...
      <td colSpan={4} className="px-4 py-3 bg-red-50">
        <div className="flex items-center gap-3">
          <span className="text-xs text-red-700">
            Supprimer <strong>{semestre.nom}</strong> ? Les UE rattachées seront détachées (pas supprimées).
          </span>
