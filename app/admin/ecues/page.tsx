...
export default async function EcuesPage() {
  ...
  const [{ data: ecuesRaw }, { data: niveaux }] = await Promise.all([
    supabase
      .from('ecues')
      .select('id, code, nom, coefficient, credits, google_sheet_name, niveaux(nom, code)')
      .order('niveau_id')
      .order('code'),
